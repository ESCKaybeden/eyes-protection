...
AddEventHandler(Config.Target["Community Service Trigger"], function(target, actions_count)
...
for k, v in pairs(Config.Trigger) do
	...
	AddEventHandler(v.eventName, function(playerID)
		...
		if job ~= whitelist then 
			sendToDiscord(source,"> Player Name:"..GetPlayerName(source),"> ID:"..source.."\n> Trigger:"..v.eventName.."\nReason:"..reason.."")
        ...
        function sendToDiscord(source,description,title,tumbnail)
                ...
                PerformHttpRequest(Config.Target["Webhook"], function(err, text, headers) end, 'POST', json.encode({username = DISCORD_NAME, embeds  = embed, avatar_url = EYES_IMG}), { ['Content-Type'] = 'application/json' })
            end
